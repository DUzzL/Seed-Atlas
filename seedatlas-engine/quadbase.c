...
int scanForQuadBits(const StructureConfig sconf, int radius, uint64_t s48,
        ...
        int64_t w, int64_t h, Pos *qplist, int n, const volatile char *abort)
{
        ...
        const volatile char *abort)
{
    ...
    for (i = 0; lowBits[i]; i++)
    {
        ...
        cnt += scanForQuadBits(sconf, radius, s48, lowBits[i]-salt, lowBitN, invB,
                x, z, w, h, qplist+cnt, n-cnt, abort);
        if (cnt >= n)
            break;
    }
...
int scanForQuads(
        ...
        int x, int z, int w, int h, Pos *qplist, int n)
{
    return scanForQuadsCancelable(sconf, radius, s48, lowBits, lowBitN, salt,
            x, z, w, h, qplist, n, NULL);
}
