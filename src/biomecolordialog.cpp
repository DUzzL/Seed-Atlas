...
{
    const QString requested = qvariant_cast<QString>(ui->comboColormaps->itemData(index));
    if (modified)
    {
        if (activerc.isEmpty())
        {
            {
                QSignalBlocker blocker(ui->comboColormaps);
                ui->comboColormaps->setCurrentIndex(
                    ui->comboColormaps->findData(activerc));
            }
            if (!saveAsColormap(false))
                return;
        }
        else if (saveColormap(activerc, "") < 0)
        {
            warn(this, tr("Failed to save biome colors."));
            QSignalBlocker blocker(ui->comboColormaps);
            ui->comboColormaps->setCurrentIndex(
                ui->comboColormaps->findData(activerc));
            return;
...

    {
        QSignalBlocker blocker(ui->comboColormaps);
        ui->comboColormaps->setCurrentIndex(
            ui->comboColormaps->findData(requested));
    }
    ui->buttonRemove->setEnabled(!requested.isEmpty());
    activerc = requested;
    if (!activerc.isEmpty())
    {
        QFile file(activerc);
...
bool BiomeColorDialog::saveAsColormap(bool closeAfter)
{
    QString rc;
    int n;
    ...
    if (!QDir().mkpath(path))
    {
        warn(this, tr("Failed to create the configuration directory."));
        return false;
    }
    for (n = 1; n < 1000; n++)
    {
    ...
    bool ok = false;
    QString text = QInputDialog::getText(this, tr("Save biome colors as..."),
