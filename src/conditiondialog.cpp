...
bool ConditionDialog::resolveLuaChanges()
{
    if (!ui->textEditLua->document()->isModified())
        return true;

    int button = QMessageBox::warning(this, tr("Unsaved changes"),
        tr("Save changes to the Lua script?"),
    ...
    return !ui->textEditLua->document()->isModified();
}

void ConditionDialog::on_comboLua_currentIndexChanged(int)
{
    const int previous = ui->comboLua->findData(QVariant::fromValue(luahash));
    if (!resolveLuaChanges())
    {
        QSignalBlocker blocker(ui->comboLua);
        ui->comboLua->setCurrentIndex(previous);
        return;
    }
...
void ConditionDialog::on_pushInfoLua_clicked()
{
    ...
    mb->setText( tr(
        ...
        "and structure type enums from the Seed Atlas engine, which means they can be "
