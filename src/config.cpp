...
bool MapConfig::sameMapOpts(const MapConfig& a) const
{
    ...
    {
        if (opts[i].scale != a.opts[i].scale ||
            opts[i].enabled != a.opts[i].enabled)
            return false;
    }
    return true;
}
...
void Config::load(QSettings& settings)
{
    ...
    uistyle = qBound(int(STYLE_SYSTEM),
        settings.value("config/uistyle", uistyle).toInt(), int(STYLE_LIGHT));
    maxMatching = qBound(1, settings.value("config/maxMatching", maxMatching).toInt(), 99999999);
    gridSpacing = std::max(0, settings.value("config/gridSpacing", gridSpacing).toInt());
    gridMultiplier = settings.value("config/gridMultiplier", gridMultiplier).toInt();
    if (gridMultiplier != 2 && gridMultiplier != 4 &&
    ...
    if (!std::isfinite(iconScale))
        iconScale = 1.0;
    iconScale = qBound<qreal>(1.0/8, iconScale, 16.0);
}
