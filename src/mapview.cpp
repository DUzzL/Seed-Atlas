...
{
    updatecounter = std::max(updatecounter, 1);
    scheduleFrame();
}

void MapView::scheduleFrame()
{
...
void MapView::paintEvent(QPaintEvent *)
{
    ...
    if (world)
    {
        ...
        if (holding)
        {
            motionx = focusx - prevx;
            motionz = focusz - prevz;
        }
        else if (anielapsed.isValid())
        {
            motionx = x_dst - x_src;
            motionz = z_dst - z_src;
        }
        const qreal motionLength = std::hypot(motionx, motionz);
        if (motionLength > 0)
        {
