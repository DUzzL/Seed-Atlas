...
bool Condition::readHex(const QString& hex)
{
    ...
    const size_t required = offsetof(Condition, count);
    if ((encoded.size() & 1) || (size_t)encoded.size() / 2 < required)
        return false;
    const QByteArray ba = QByteArray::fromHex(encoded);
    if (ba.size() * 2 != encoded.size() || (size_t)ba.size() < required)
        return false;
...
QString ConditionTree::set(const std::vector<Condition>& cv, int mc)
{
    ...
    for (const Condition& c : cv)
    {
        if (c.meta & Condition::DISABLED)
            continue;
        if (c.save < 1 || c.save > 99 || c.type < 0 || c.type >= FILTER_MAX)
            return QApplication::translate("Filter", "invalid condition data");
        if (c.save > cmax)
            cmax = c.save;
    }
    condvec.clear();
    ...
    for (const Condition& c : cv)
    {
        ...
        if (!err.isEmpty())
            return err;
        if (c.relative < 0 || c.relative > cmax)
            return QApplication::translate("Filter", "invalid condition reference");
...
QString SearchThreadEnv::init(int mc, bool large, const ConditionTree& condtree)
{
    ...
    for (const Condition& c: condtree.condvec)
    {
        ...
        auto old = l_states.find(c.hash);
        if (old != l_states.end())
            lua_close(old->second);
        l_states[c.hash] = L;
    }
    return "";
}
...
static QVector<Pos> scanAllQuadCandidates(const StructureConfig& sconf,
        int radius, uint64_t seed, const uint64_t *lowBits, int lowBitN,
        int rx, int rz, int rw, int rh, const volatile char *abort)
{
    struct ScanArea { int x, z, w, h; };
    QVector<Pos> found;
    if (rw < 0 || rh < 0)
        return found;

    std::vector<ScanArea> pending = {{rx, rz, rw, rh}};
    std::vector<Pos> buffer(1000);
    while (!pending.empty() && !(abort && *abort))
    {
        const ScanArea area = pending.back();
        pending.pop_back();
        const int count = scanForQuadsCancelable(sconf, radius, seed & MASK48,
            lowBits, lowBitN, sconf.salt, area.x, area.z, area.w, area.h,
            buffer.data(), int(buffer.size()), abort);
        if (abort && *abort)
            break;
        if (count == int(buffer.size()))
        {
            // A full buffer is ambiguous: it may be exactly full or truncated.
            // Split into disjoint inclusive ranges and rescan both halves.
            if (area.w >= area.h && area.w > 0)
            {
                const int first = (area.w + 1) / 2;
                pending.push_back({area.x + first, area.z,
                    area.w - first, area.h});
                pending.push_back({area.x, area.z, first - 1, area.h});
                continue;
            }
            if (area.h > 0)
            {
                const int first = (area.h + 1) / 2;
                pending.push_back({area.x, area.z + first,
                    area.w, area.h - first});
                pending.push_back({area.x, area.z, area.w, first - 1});
                continue;
            }

            // A single region normally has at most one candidate. Keep the
            // routine complete even if a future low-bit table changes that.
            if (buffer.size() <= size_t(INT_MAX / 2))
            {
                buffer.resize(buffer.size() * 2);
                pending.push_back(area);
                continue;
            }
        }
        found.reserve(found.size() + count);
        for (int i = 0; i < count; i++)
            found.push_back(buffer[i]);
    }
    return found;
}


void findQuadStructsInRegions(int styp, Generator *g, QVector<QuadInfo> *out,
        int rx, int rz, int rw, int rh, const volatile char *abort)
{
    StructureConfig sconf;
    ...
    if (styp == Swamp_Hut)
    {
        const QVector<Pos> qlist = scanAllQuadCandidates(sconf, 128, g->seed,
            low20QuadHutBarely, 20, rx, rz, rw, rh, abort);
        ...
        for (Pos qr : qlist)
        {
            ...
            // Keep the analysis definition identical to the quad-base
            // conditions used by seed finding: these are four sufficiently
            // close structure attempts, independent of biome viability.
            QuadInfo qinfo;
            for (int j = 0; j < 4; j++)
                qinfo.p[j] = qs[j];
            qinfo.c = (moveStructure(g->seed, -qr.x, -qr.z) + sconf.salt) & 0xfffff;
            switch (getQuadHutCst(qinfo.c))
            {
            case CST_IDEAL:   qinfo.flt = F_QH_IDEAL; break;
            case CST_CLASSIC: qinfo.flt = F_QH_CLASSIC; break;
            case CST_NORMAL:  qinfo.flt = F_QH_NORMAL; break;
            default:          qinfo.flt = F_QH_BARELY; break;
            }
            qinfo.typ = styp;
    ...
    {
        const QVector<Pos> qlist = scanAllQuadCandidates(sconf, 160, g->seed,
            g_qm_90, 48, rx, rz, rw, rh, abort);
        ...
        for (Pos qr : qlist)
        {
            ...
            for (int j = 0; j < 4; j++)
                qinfo.p[j] = qs[j];
            qinfo.c = (moveStructure(g->seed, -qr.x, -qr.z) + sconf.salt) & MASK48;
            qinfo.flt = qmonumentQual(qinfo.c) >= 58*58*4*95/100
                ? F_QM_95 : F_QM_90;
            qinfo.typ = styp;
...
{
    const int r = 30000000 / 512;
    findQuadStructsInRegions(styp, g, out, -r, -r, 2*r, 2*r);
}
