...
: QThread(parent), wi(),stop(false),sidx(),pidx(),workDone()
{
    env.stop = &stop;
}
...
{
    workers.reset();
    const uint64_t total = uint64_t(seeds.size()) * pos.size();
    workDone = 0;
    std::atomic_uint64_t next(0);
    auto scan = [&](size_t) {
        SearchThreadEnv localEnv;
        if (!localEnv.init(wi.mc, wi.large, condtree).isEmpty())
            return;
        localEnv.stop = &stop;
        uint64_t activeSeed = 0;
        bool hasActiveSeed = false;
        while (!stop.load(std::memory_order_relaxed))
        {
            const uint64_t index = next.fetch_add(1, std::memory_order_relaxed);
            if (index >= total)
                break;
            const size_t seedIndex = size_t(index / pos.size());
            const size_t posIndex = size_t(index % pos.size());
            const uint64_t seed = seeds[seedIndex];
            sidx = long(seedIndex);
            pidx = long(posIndex);
            if (!hasActiveSeed || activeSeed != seed)
            {
                localEnv.setSeed(seed);
                activeSeed = seed;
                hasActiveSeed = true;
            }
            ...
            if (testTreeAt(at, &localEnv, PASS_FULL_64, cpos)
                != COND_OK)
            {
    ...
    runMappingWorkers(total, workers, scan);
}
...
void TabLocations::onProgressTimeout()
{
    ...
    size_t progress = thread.workDone.load(std::memory_order_relaxed);
    double frac = total ? progress / (double)total : 0.0;
    ui->labelStatus->setText(QString::asprintf("%zu / %zu (%.1f%%), %zu threads",
        progress, total, 100.0*frac, thread.workers.peak.load(std::memory_order_relaxed)));
    if (parent)
...
void TabLocations::on_pushStart_clicked()
{
    ...
    auto appendPosition = [&](qint64 px, qint64 pz) {
        if (px < std::numeric_limits<int>::min() ||
            px > std::numeric_limits<int>::max() ||
            pz < std::numeric_limits<int>::min() ||
            pz > std::numeric_limits<int>::max())
        {
            coordinatesValid = false;
            return;
        }
        thread.pos.push_back(Pos{int(px), int(pz)});
    };
    ...
    if (mode == SMODE_RADIAL_GRID)
    {
        ...
        appendPosition(x0, z0);

        for (uint64_t i = 0; i < v.size() && coordinatesValid; i++)
        {
            qint64 x = qint64(a) * v[i].x;
            qint64 z = qint64(a) * v[i].z;

            if (z == 0 || x == z)
            {
                appendPosition(qint64(x0)+x, qint64(z0)+z);
                appendPosition(qint64(x0)-z, qint64(z0)+x);
                appendPosition(qint64(x0)-x, qint64(z0)-z);
                appendPosition(qint64(x0)+z, qint64(z0)-x);
            }
            else
    ...
    else if (mode == SMODE_SQUARE_SPIRAL)
    {
        ...
        {
            appendPosition(qint64(a)*rx + x0, qint64(a)*rz + z0);
            if (!coordinatesValid)
                break;
            rx += dx;
            rz += dz;
    ...
    else if (mode == SMODE_NORMAL)
    {
        ...
        for (uint64_t j = 0; j < n; j++)
        {
            double x = round(a * norm(rng.mt)) + x0;
            double z = round(a * norm(rng.mt)) + z0;
            ...
            appendPosition(qint64(x), qint64(z));
        }
    }
    ...
    {
        thread.pos.clear();
        warn(this, tr("The sampling pattern exceeds the supported coordinate range."));
        return;
    }
