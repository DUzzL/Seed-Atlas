...
// Mojang's worldgen noise registry; the implementation uses the bundled engine noise.
...
void Level::resizeLevel(std::vector<Quad*>& cache, int64_t x, int64_t z, int64_t w, int64_t h)
{
    ...
    for (int j = 0; j < h; j++)
    {
        for (int i = 0; i < w; i++)
        {
            ...
            if (g == NULL)
            {
                g = world->takeCachedQuad(cache, blocks, sopt, dim, x+i, z+j);
            }
            if (g == NULL)
            {
        ...
        qreal motionx, qreal motionz)
{
    ...
    if ((nti1-nti0)*2 < tw || nti0 < tx || nti1 > tx+tw || ntj0 < tz || ntj1 > tz+th)
    {
        ...
        qreal padz = 0.2 * (bz1 - bz0);
        const qreal biasx = 0.5 * padf * qBound<qreal>(-1, motionx, 1);
        const qreal biasz = 0.5 * padz * qBound<qreal>(-1, motionz, 1);
        nti0 = (int) std::floor((bx0-padf+biasx) / blocks);
        ntj0 = (int) std::floor((bz0-padz+biasz) / blocks);
        nti1 = (int) std::floor((bx1+padf+biasx) / blocks) + 1;
        ntj1 = (int) std::floor((bz1+padz+biasz) / blocks) + 1;
...
QWorld::QWorld(WorldInfo wi, int dim, LayerOpt lopt)
    ...
    , mutex()
    , queue()
    , workers(std::max(1, QThread::idealThreadCount()))
    , threadlimit()
    , showBB()
    , gridspacing()
    , gridmultiplier()
...
int QWorld::getBiome(Pos p)
{
    ...
    if (activelv >= 0 && activelv < (int)lvb.size())
    {
        ...
        p.x = floorDiv(p.x, scale);
        p.z = floorDiv(p.z, scale);
        if (scale != 1)
            y /= 4;
    }
...
QuadCacheIndex& QWorld::cacheIndex(std::vector<Quad*>& cache)
{
    return &cache == &cachedbiomes ? cachedBiomeIndex : cachedStructIndex;
}

void QWorld::cacheQuad(std::vector<Quad*>& cache, Quad *quad)
{
    if (take(quad))
        quad->stopped = true;
    QuadCacheIndex& index = cacheIndex(cache);
    const QuadCacheKey key = {quad->blocks, quad->sopt, quad->dim, quad->ti, quad->tj};
    const size_t position = cache.size();
    cache.push_back(quad);
    index.insert_or_assign(key, CachedQuad{quad, position});
}

Quad *QWorld::takeCachedQuad(std::vector<Quad*>& cache, int blocks, int sopt,
        int dim, int ti, int tj)
{
    QuadCacheIndex& index = cacheIndex(cache);
    const QuadCacheKey key = {blocks, sopt, dim, ti, tj};
    auto found = index.find(key);
    if (found == index.end())
        return nullptr;
    const CachedQuad cached = found->second;
    index.erase(found);
    if (cached.index < cache.size() && cache[cached.index] == cached.quad)
        cache[cached.index] = nullptr;
    return cached.quad;
}

void QWorld::rebuildCacheIndex(std::vector<Quad*>& cache)
{
    QuadCacheIndex& index = cacheIndex(cache);
    index.clear();
    index.reserve(cache.size());
    for (size_t position = 0; position < cache.size(); position++)
    {
        Quad *quad = cache[position];
        if (!quad)
            continue;
        const QuadCacheKey key = {quad->blocks, quad->sopt, quad->dim,
            quad->ti, quad->tj};
        index.insert_or_assign(key, CachedQuad{quad, position});
    }
}
...
void QWorld::cleancache(std::vector<Quad*>& cache, unsigned int maxsize)
{
    ...
    if (live < maxsize && live == cache.size())
        return;

    const size_t targetsize = 4 * size_t(maxsize) / 5;

    mutex.lock();
    std::vector<Quad*> newcache;
    newcache.reserve(std::min(live, targetsize));
    size_t remaining = live;
    for (Quad *q : cache)
    {
        if (!q)
            continue;
...
void QWorld::draw(QPainter& painter, int vw, int vh, qreal focusx, qreal focusz,
        qreal blocks2pix, qreal motionx, qreal motionz)
{
    ...
    for (int sopt = D_DESERT; sopt < D_SPAWN; sopt++)
    {
        ...
        if (l.vis <= blocks2pix && sshow[sopt] && dim == l.dim)
            l.update(cachedstruct, bx0, bz0, bx1, bz1, motionx, motionz);
    ...
    for (int li = lvb.size()-1; li >= 0; --li)
    {
        ...
        if (li == activelv || li == activelv+1)
            l.update(cachedbiomes, bx0, bz0, bx1, bz1, motionx, motionz);
        else
            l.setInactive(cachedbiomes);
    }
