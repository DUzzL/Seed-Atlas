...
struct QuadCacheKey
{
    int blocks, sopt, dim, ti, tj;

    bool operator==(const QuadCacheKey& other) const
    {
        return blocks == other.blocks && sopt == other.sopt && dim == other.dim &&
            ti == other.ti && tj == other.tj;
    }
};

struct QuadCacheKeyHash
{
    size_t operator()(const QuadCacheKey& key) const
    {
        size_t hash = 1469598103934665603ULL;
        const int values[] = {key.blocks, key.sopt, key.dim, key.ti, key.tj};
...
using QuadCacheIndex = std::unordered_map<QuadCacheKey, CachedQuad, QuadCacheKeyHash>;
...
public:
    ...
    void draw(QPainter& painter, int vw, int vh, qreal focusx, qreal focusz,
        qreal blocks2pix, qreal motionx = 0, qreal motionz = 0);
    ...
    void cacheQuad(std::vector<Quad*>& cache, Quad *quad);
    Quad *takeCachedQuad(std::vector<Quad*>& cache, int blocks, int sopt,
        int dim, int ti, int tj);
    QuadCacheIndex& cacheIndex(std::vector<Quad*>& cache);
    void rebuildCacheIndex(std::vector<Quad*>& cache);
...
public:
    ...
    QuadCacheIndex cachedBiomeIndex;
    QuadCacheIndex cachedStructIndex;
    uint64_t memlimit;
    QMutex mutex;
    Scheduled *queue;
